...
# BUILD: docker build --rm -t tokimasa/docker-airflow .
# SOURCE: https://github.com/tokimasa/docker-airflow

FROM python:3.7-slim-buster
LABEL maintainer="Tokimasa"
...
RUN set -ex \
	...
	&& pip install --no-cache-dir mysql-connector-python==8.0.25 \
	&& pip install --no-cache-dir pandas==1.2.4 \
	&& pip install --no-cache-dir numpy==1.20.3 \
	&& pip install --no-cache-dir scikit_learn==0.22.1 \
	&& pip install --no-cache-dir yellowbrick==1.3.post1 \
